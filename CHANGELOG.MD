# 5.4
*yet another formatting/code rework*

* **[Hbm's NTM / NTM:Space]** Fixed a **long-standing** issue with shadow distortion / self-shadowing (dark spots) on large machines (`NTM_SHADOW_FIX`)
* **[Main]** Removed placeholder fixes (journeymap / `glClipPlane` in NTM), as these are now fixed properly in angelica

# 5.3
* **[Minecraft]** Fixed particles rendering during shadow pass
* **[Hbm's NTM]** Fixed crash with X5634+
* **[Hbm's NTM]** Fixed health bars / thermal sights / bullets rendering during shadow pass
* **[Hbm's NTM / Techguns]** Improved texture (black background) fixes

# 5.2
* **[Main]** Fixed translucent particles becoming very dark with Complementary shaders enabled (e.g. cooling tower steam particles from Hbm's NTM)
* **[Hbm's NTM]** Fixed crash when rendering guns with X5615+

# 5.1
* **[Minecraft]** Fixed experience bar black background if there is an item in the hotbar (`VANILLA_XP_BAR_ALPHA_FIX`)

# 5.0
* **Basically a complete rework, see the mod description**

# 4.9-patch-1
*Update to this version only if you're using Angelica beta68+, otherwise it's not necessary*

* **[Hbm's NTM:Space]** Skybox fixes have been removed - they're now part of Angelica (starting with beta68)
* **[Main]** `VANILLA_DISABLE_HORIZON` option has been also removed - it's (also) part of Angelica now (Notfine in the future)

# 4.9
* **[Electrical Age (ELN)]** Fixed crash when rendering data logs
* **[Electrical Age (ELN) && OpenComputers]** Added a workaround(!) for a visual bug with Angelica shaders
  - May significantly reduce performance (disables the use of display lists)
  - Config Electrical Age - `minecraft\config\shaderfixer\mixinsLate_GENERAL.cfg -> ELN_DISABLE_DL`
  - Config OpenComputers - `minecraft\config\shaderfixer\mixinsLate_GENERAL.cfg -> OC_DISABLE_DL`

# 4.8-patch-1
* **[Hbm's NTM]** Fixed crash when rendering RBMK console
  - ouch
...
* **[Hbm's NTM / NTM:Space]** Fixed brightness of various beams with shaders
* **[Main]** Fixed ~~peak mojang coding~~ graphical artifacts when rendering nicknames with shaders
* **[Avaritia]** Fixed rendering with Angelica (shaders)
  - Requires Angelica 1.0.0-beta60 (yes I'm from future)
  - Works with Complementary 5.x and BSL 8.x (other shaders may have flickering!)
