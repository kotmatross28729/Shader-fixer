...
public class MixinModelBiped extends ModelBase {
    ...
    @ModifyConstant(method = "setRotationAngles"
            , constant = @Constant(floatValue = -((float) Math.PI * 2F / 5F)))
    public float transformLegRotateAngleX(float angle) {
        // 20 in vanilla (or not, the fuck is that 1.4137167),
        // but at 20, there are still triangles on the very edge (so 18 (1.5707963))
        return ((float) (-Math.PI * 9F / 18F)); 
    }
