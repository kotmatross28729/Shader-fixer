...
private static GoreData shaders_fixer$getGoreData(EntityLivingBase entityClass) {
        ...
        for (String entity : TechgunsGoreList) {
            ...
            if (EntityList.getEntityString(entityClass).equals(entityname)) {
                genericGore = new GoreData(modelBiped, numGibs, new ResourceLocation("techguns", "textures/entity/gore.png"), scale, bloodColorRed, bloodColorGreen, bloodColorBlue);
            }  else {
                genericGore = new GoreData(modelBiped, numGibs, new ResourceLocation("techguns", "textures/entity/gore.png"), 1.0F, 255, 255, 255);
            }
        }
            if (entityClass instanceof EntityPlayer) {
                genericGore = new GoreData(modelBipedPlayer, 6, new ResourceLocation("techguns", "textures/entity/gore.png"), 0.66F, 160, 21, 31);
            } else if (entityClass.getClass() == EntityZombie.class) {
                genericGore = new GoreData(modelBiped, 6, new ResourceLocation("textures/entity/zombie/zombie.png"), 0.66F, 110, 21, 41);
            } else if (entityClass.getClass() == ZombieSoldier.class) {
                genericGore = new GoreData(modelBiped, 6, new ResourceLocation("techguns", "textures/entity/zombie_soldier.png"), 0.66F, 110, 21, 41);
            } else if (entityClass.getClass() == ArmySoldier.class) {
                genericGore = new GoreData(modelBiped, 6, new ResourceLocation("techguns", "textures/entity/army_soldier.png"), 0.66F, 160, 21, 31);
                ...
                genericGore = new GoreData(modelBiped, 6, new ResourceLocation("textures/entity/zombie_pigman.png"), 0.66F, 110, 51, 11);
                ...
                genericGore = new GoreData(modelBiped, 6, new ResourceLocation("techguns", "textures/entity/bandit.png"), 0.66F, 160, 21, 31);
    ...
    public static void createDeathEffect(final EntityLivingBase entity, final EntityDeathUtils.DeathType deathtype, final float motionX, final float motionY, final float motionZ) {
        ...
        if (deathtype == EntityDeathUtils.DeathType.GORE) {
            ...
            ClientProxy.spawnParticle((EntityFX)new EntityBloodEffect(entity.worldObj, x, y, z, motionX, motionY, motionZ, entity.width, entity.height, data));
            for (int count = data.numGibs, i = 0; i < count; ++i) {
                ...
                final FlyingGibs ent = new FlyingGibs(entity.worldObj, entity, data, x, y, z, motionX * 0.35 + vx, motionY * 0.15 + vy, motionZ * 0.35 + vz, (entity.width + entity.height) / 2.0f, i);
